    ...
    <div class="">
        ...
        <div class="row frmcontent">
            ...
            <form action="{{ route('admin.product.sua', $product->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <select class="form-select" id="floatingSelect" aria-label="Floating label select example"
                        ...
                        @if ($listCate->count() > 0)
                            @foreach ($listCate as $catee)
                                <option {{ $catee->id == $product->cate ? 'selected' : '' }} value="{{ $catee['id'] }}">
                                    {{ $catee['name'] }}</option>
                            @endforeach
                        @endif
                    </select>
                ...
                <div class="mb10">
                    Số lượng<br>
                    <input type="text" value="{{ $product->number }}" name="soluong">
                </div>
                <div class="mb10">
                    Hình<br>
                    ...
                    <select name="sizes[]" multiple="multiple" id="" class="tag_select2_choose">
                        @if ($sizes->count() > 0)
                            @foreach ($sizes as $size)
                                <option
                                    {{ in_array($size->slug, json_decode(!empty($product->property->sizes) ? $product->property->sizes : json_encode([]), true)) ? 'selected' : '' }}
                                    value={{ $size->slug }}>
                    ...
                    <select name="colors[]" multiple="multiple" id="" class="tag_select2_colors">
                        @if ($colors->count() > 0)
                            @foreach ($colors as $color)
                                <option
                                    {{ in_array($color->slug, json_decode(!empty($product->property->colors) ? $product->property->colors : json_encode([]), true)) ? 'selected' : '' }}
                                    value={{ $color->slug }}>
